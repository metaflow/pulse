...
void MoveGenerator::getLegalMoves(Position& position, int depth, bool isCheck,
                                  MoveEntryList& legalMoves) {
...
bool MoveGenerator::isValidMove(Position& position, bool isCheck, int move) {
  position.makeMoveR(move);
  bool f = !position.isCheck(Color::opposite(position.activeColor));
  position.undoMoveR(move);
  return f;
}

void MoveGenerator::getMoves(Position& position, int depth, bool isCheck,
                             MoveEntryList& moves) {
  moves.size = 0;
  ...
  if (depth > 0) {
    ...
    // if (!isCheck) {
    // int square =
    // Bitboard::next(position.pieces[position.activeColor][PieceType::KING]);
...
void MoveGenerator::addMoves(MoveEntryList& list, int originSquare,
    ...
    while (Square::isValid(targetSquare)) {
      ...
      if (targetPiece == Piece::NOPIECE) {
        ...
        list.entries[list.size++].move =
            Move::valueOf(MoveType::NORMAL, originSquare, targetSquare,
                          originPiece, Piece::NOPIECE, PieceType::NOPIECETYPE);
        ...
        if (Piece::getColor(targetPiece) == oppositeColor) {
          ...
          list.entries[list.size++].move =
              Move::valueOf(MoveType::NORMAL, originSquare, targetSquare,
                            originPiece, targetPiece, PieceType::NOPIECETYPE);
        }
...
void MoveGenerator::addPawnMoves(MoveEntryList& list, int pawnSquare,
                                 Position& position) {
      ...
      if (targetPiece != Piece::NOPIECE) {
        if (Piece::getColor(targetPiece) == Color::opposite(pawnColor)) {
          ...
          if ((pawnColor == Color::WHITE &&
               Square::getRank(targetSquare) == Rank::r8) ||
              (pawnColor == Color::BLACK &&
               Square::getRank(targetSquare) == Rank::r1)) {
            // Pawn promotion capturing move

            list.entries[list.size++].move =
                Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                              pawnPiece, targetPiece, PieceType::QUEEN);
            list.entries[list.size++].move =
                Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                              pawnPiece, targetPiece, PieceType::ROOK);
            list.entries[list.size++].move =
                Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                              pawnPiece, targetPiece, PieceType::BISHOP);
            list.entries[list.size++].move =
                Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                              pawnPiece, targetPiece, PieceType::KNIGHT);
          } else {
            // Normal capturing move

            list.entries[list.size++].move =
                Move::valueOf(MoveType::NORMAL, pawnSquare, targetSquare,
                              pawnPiece, targetPiece, PieceType::NOPIECETYPE);
          }
        }
      } else if (targetSquare == position.enPassantSquare) {
        // En passant move
        int captureSquare =
            targetSquare + (pawnColor == Color::WHITE ? Square::S : Square::N);
        targetPiece = position.board[captureSquare];

        list.entries[list.size++].move =
            Move::valueOf(MoveType::ENPASSANT, pawnSquare, targetSquare,
                          pawnPiece, targetPiece, PieceType::NOPIECETYPE);
      }
    }
  }
  ...
  int targetSquare = pawnSquare + direction;
  if (Square::isValid(targetSquare) &&
      position.board[targetSquare] == Piece::NOPIECE) {
    if ((pawnColor == Color::WHITE &&
         Square::getRank(targetSquare) == Rank::r8) ||
        (pawnColor == Color::BLACK &&
         Square::getRank(targetSquare) == Rank::r1)) {
      ...
      list.entries[list.size++].move =
          Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                        pawnPiece, Piece::NOPIECE, PieceType::QUEEN);
      list.entries[list.size++].move =
          Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                        pawnPiece, Piece::NOPIECE, PieceType::ROOK);
      list.entries[list.size++].move =
          Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                        pawnPiece, Piece::NOPIECE, PieceType::BISHOP);
      list.entries[list.size++].move =
          Move::valueOf(MoveType::PAWNPROMOTION, pawnSquare, targetSquare,
                        pawnPiece, Piece::NOPIECE, PieceType::KNIGHT);
    } else {
      // Normal move

      list.entries[list.size++].move =
          Move::valueOf(MoveType::NORMAL, pawnSquare, targetSquare, pawnPiece,
      ...
      if (Square::isValid(targetSquare) &&
          position.board[targetSquare] == Piece::NOPIECE) {
        if ((pawnColor == Color::WHITE &&
             Square::getRank(targetSquare) == Rank::r4) ||
            (pawnColor == Color::BLACK &&
             Square::getRank(targetSquare) == Rank::r5)) {
          // Pawn double move

          list.entries[list.size++].move =
              Move::valueOf(MoveType::PAWNDOUBLE, pawnSquare, targetSquare,
                            pawnPiece, Piece::NOPIECE, PieceType::NOPIECETYPE);
        }
      }
    }
  }
...
void MoveGenerator::addCastlingMoves(MoveEntryList& list, int kingSquare,
                                     Position& position) {
  ...
  if (Piece::getColor(kingPiece) == Color::WHITE) {
    ...
    if ((position.castlingRights & Castling::BLACK_QUEENSIDE) !=
      ...
      list.entries[list.size++].move =
          Move::valueOf(MoveType::CASTLING, kingSquare, Square::c8, kingPiece,
                        Piece::NOPIECE, PieceType::NOPIECETYPE);
    }
  }
}
